    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            if self.discrete_action_space:
                u_action_space = spaces.Discrete(world.dim_p * 2 + 1)  # world.dim_p位置维度=2，如果环境维度是2，就说明有上下左右4中动作可以做，加上不懂，就是5.如果环境维度是1，就只有左右两种动作，加上不动就是3？
            else:
                u_action_space = spaces.Box(low=-agent.u_range, high=+agent.u_range, shape=(world.dim_p,), dtype=np.float32)  # 不同维度方向上施加的力？
            if agent.movable:
            ...
            if not agent.silent:  # silent属性代表不能发送信息
