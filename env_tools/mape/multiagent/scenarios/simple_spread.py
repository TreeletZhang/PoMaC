...
class Scenario(BaseScenario):
    ...
    def reward(self, agent, world):
        if agent.iden == 0: # compute this only once when called with the first agent
            ...
            self.rewards = np.full(self.num_agents, joint_reward) #生成一个长度为num_agent,每个元素值都为joint_reward的数组
    ...
    def observation(self, agent, world):
        ...
        entity_pos = [entity.state.p_pos - agent.state.p_pos for entity in world.landmarks]  # 目标实体与智能体的相对位置
