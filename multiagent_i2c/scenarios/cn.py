...
class Scenario(BaseScenario):  
    ...
    #     for entity in world.landmarks:  # world.entities: 所有地标的信息
    ...
    def observation(self, agent, world):  # 观测加速度
        # get positions of all entities in this agent's reference frame
        entity_pos = []
        dis_lm_n = []
        num_landmarks_obs = world.num_landmarks_obs
        ...
        sort_index = sorted(range(len(dis_agent_n)), key=lambda k: dis_agent_n[k])
        near_agent_pos = [other_pos[sort_index[i]] for i in range(num_agents_obs)]
        near_agent_vel = [other_vel[sort_index[i]] for i in range(num_agents_obs)]
        return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + near_lm_pos + near_agent_pos + near_agent_vel)
